...
export default function ChartPage() {
  ...
  const setChartData = useCallback((bars: BarData[]) => {
    if (!chartRef.current) { console.warn('[Chart] setChartData skipped — chart engine not ready'); return }
    console.log(`[Chart] setChartData: ${bars?.length ?? 0} bars, engine=`, !!chartRef.current)
  ...
  const handleEngineReady = useCallback((index: number, engine: ChartEngine) => {
    console.log(`[Chart] Engine ready: index=${index}, dataRef has ${dataRef.current?.length ?? 0} bars`)
    if (index === 0) {
  ...
  console.log(`[Chart] Render — loading=${loading} error=${!!error} data=${data?.length ?? 0} bars engine=${!!chartRef.current}`)
  return (
